...
class AuditReport:
    ...
    def to_dict(self, include_metadata: bool = True) -> dict[str, Any]:
        ...
        output: dict[str, Any] = {
            ...
            "probes": {
                name: result.model_dump() for name, result in self.results.items()
            },
        }
...
class DataManager:
    ...
    def __init__(self, settings: Settings) -> None:
        ...
        base_output_dir = Path(settings.output.directory)
        ...
        if use_fake:
            self.output_dir = base_output_dir / "fake"
            logger.info("FAKE mode: outputs will be saved to %s", self.output_dir)
        else:
    ...
    def save_report(self, report: AuditReport) -> Path:
        ...
        # Serialize and save
        data = report.to_dict(include_metadata=self.settings.output.include_metadata)
        ...
        try:
            ...
            raise DataManagerError(
                f"Failed to save report: {output_path}", details={"error": str(e)}
            ) from e
