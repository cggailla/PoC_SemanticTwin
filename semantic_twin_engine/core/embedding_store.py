...
class FakeEmbeddingProvider:
    ...
    def generate_embedding(text: str, model: str) -> np.ndarray:
        ...
        Args:
            ...
            model: The model name (for salt consistency).
...
class EmbeddingStore:
    ...
    def _load_cache(self) -> None:
        ...
        if self._embeddings_path.exists():
            try:
                ...
                logger.info(
                    "Loaded %d embeddings from cache", len(self._embeddings_cache)
                )
        ...
        if self._centroids_path.exists():
            try:
                ...
                logger.info(
                    "Loaded %d centroids from cache", len(self._centroids_cache)
                )
            except Exception as e:
    ...
    def _fetch_embeddings(self, texts: list[str]) -> list[np.ndarray]:
        """Fetch embeddings from OpenAI API or return fake embeddings if configured.

        If FAKE_OPENAI_RESULT=True, returns deterministic fake embeddings.
        ...
        if use_fake:
            logger.info(
                "Using FAKE embeddings for %d texts (FAKE_OPENAI_RESULT=True)",
                len(texts),
            )
            return [
                FakeEmbeddingProvider.generate_embedding(text, self.model)
                for text in texts
            ]
    ...
    def get_embeddings(self, texts: list[str]) -> list[np.ndarray]:
        ...
        if cache_hits > 0:
            logger.info(
                "Embedding cache: %d hits, %d misses", cache_hits, len(texts_to_fetch)
            )
    ...
    def invalidate_dimension(self, dimension: str) -> None:
        ...
        """
        keys_to_remove = [
            k for k in self._centroids_cache if k.startswith(f"{dimension}_")
        ]
